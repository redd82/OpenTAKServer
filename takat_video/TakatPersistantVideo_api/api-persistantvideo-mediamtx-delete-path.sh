#!/bin/bash
# ------------------------------------------------------------------
# Script to call MediaMTX_DeletePath via TakatVideo API (with login)
# ------------------------------------------------------------------
...
PATH_ENDPOINT="/api/persistantvideo/MediaMTX/DeletePath"

LOGIN_URL="http://${HOST}:${PORT}${LOGIN_ENDPOINT}"
PATH_URL="http://${HOST}:${PORT}${PATH_ENDPOINT}"
...
RESPONSE=$(curl -s -X POST "$PATH_URL" \
  -H "Content-Type: application/json" \
