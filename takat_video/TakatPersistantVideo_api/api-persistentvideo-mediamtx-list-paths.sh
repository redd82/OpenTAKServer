#!/bin/bash
# ------------------------------------------------------------------
# Script to call MediaMTX_ListPaths via TakatVideo API (with login)
# ------------------------------------------------------------------
...
ADD_PATH_ENDPOINT="/api/persistantvideo/MediaMTX/GetAllPaths"
...
# ------------------------------------------------------------
# 2. Prompt for ListPaths parameters
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Start base JSON
PAYLOAD="{\"port\":$PORT_MMTX,\"retries\":$RETRIES,\"verified_ssl\":$VERIFIED_SSL"
...
RESPONSE=$(curl -s -X GET "$ADD_PATH_URL" \
  -H "Content-Type: application/json" \
